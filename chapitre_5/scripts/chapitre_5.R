...
p2 <- ggplot(param.pop)+
  aes(x=n.ind,y=sd)+
  geom_line()+
  geom_hline(yintercept=sd(pop10000),linetype="dashed",col="red")+
  facet_wrap(~taille.pop)+
...
ggplot(marees)+
  ...
  geom_smooth(data=tide.obs,aes(x=echt,y=obs.tide),col="red",size=0.5,se=F)+
  theme_light(base_size = 16)


ggsave("outputs/C5F24.png", width = 20, height = 10)
